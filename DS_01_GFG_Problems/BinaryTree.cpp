...
vector<struct BinaryTreeNode*> buildAllBSTForNumbers1ToN(int start,int end){

	vector<struct BinaryTreeNode*> list;
	int i,j,k;
	...
	for(i=start;i<=end;i++){
		vector<struct BinaryTreeNode*> left=buildAllBSTForNumbers1ToN(start,i-1);
		vector<struct BinaryTreeNode*> right=buildAllBSTForNumbers1ToN(i+1,end);


		for(j=0;j<left.size();j++){

			struct BinaryTreeNode* leftnode=left[j];

			for(k=0;k<right.size();k++){

				struct BinaryTreeNode* root=getBinaryTreeNode(i);
				root->left=leftnode;
				root->right=right[k];
				list.push_back(root);
...
void closestLeafFromGivenNodeUtil(struct BinaryTreeNode* topRoot,struct BinaryTreeNode* root,int key ,int* minDist,struct BinaryTreeNode **cleaf){
